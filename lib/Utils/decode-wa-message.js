...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    // participant_pn is the real phone number JID, always prefer it over raw participant
    const participant = stanza.attrs.participant_pn || stanza.attrs.participant;
    const recipient = stanza.attrs.recipient_pn || stanza.attrs.recipient;
    const isMe = (jid) => areJidsSameUser(jid, meId);
    const isMeLid = (jid) => areJidsSameUser(jid, meLid);
    // resolve from: if it's a LID, use sender_pn instead
    const resolvedFrom = isLidUser(from)
        ? (stanza.attrs.sender_pn || stanza.attrs.from_pn || from)
        : from;
    if (isPnUser(resolvedFrom)) {
        if (recipient) {
            if (!isMe(resolvedFrom)) {
    ...
    else if (isLidUser(resolvedFrom)) {
        // LID chat with no PN available - keep as-is, will be fixed by cleanMessage
        if (recipient) {
            if (!isMeLid(from)) {
                throw new Boom('receipient present, but msg not from me', { data: stanza });
            }
        ...
        if (!stanza.attrs.participant && !stanza.attrs.participant_pn) {
    ...
    const fromMe = isJidNewsletter(from)
        ...
        : isMe(stanza.attrs.participant_pn || stanza.attrs.participant || stanza.attrs.from)
    ...
    const key = {
        ...
        ...(participant !== undefined && {
            participant: fromMe ? meId : participant
        })
    };
    const fullMessage = {
        key,
