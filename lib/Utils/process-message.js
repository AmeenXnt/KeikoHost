...
export const cleanMessage = async (message, meId, meLid, lidMappingStore) => {
    // Resolve a LID to a real phone JID using the mapping store
    const resolveJid = async (jid) => {
        if (!jid) return jid;
        if (isLidUser(jid) && lidMappingStore) {
            const pn = await lidMappingStore.getPNForLID(jid);
            if (pn) return jidNormalizedUser(pn);
        }
        return jidNormalizedUser(jid);
    };
    // Fix top-level key
    message.key.remoteJid = await resolveJid(message.key.remoteJid);
    message.key.participant = message.key.participant ? await resolveJid(message.key.participant) : undefined;
    // Recursively fix all contextInfo in the message (mentions, quoted participant/remoteJid)
    const fixContextInfo = async (ctxInfo) => {
        if (!ctxInfo) return;
        if (Array.isArray(ctxInfo.mentionedJid)) {
            ctxInfo.mentionedJid = await Promise.all(ctxInfo.mentionedJid.map(resolveJid));
            ctxInfo.mentionedJid = ctxInfo.mentionedJid.filter(Boolean);
        }
        if (ctxInfo.participant) {
            ctxInfo.participant = await resolveJid(ctxInfo.participant);
        }
        if (ctxInfo.remoteJid) {
            ctxInfo.remoteJid = await resolveJid(ctxInfo.remoteJid);
        }
    };
    const fixAllContextInfo = async (msg) => {
        if (!msg || typeof msg !== 'object') return;
        for (const key of Object.keys(msg)) {
            const val = msg[key];
            if (!val || typeof val !== 'object') continue;
            if (key === 'contextInfo') {
                await fixContextInfo(val);
            } else {
                await fixAllContextInfo(val);
            }
        }
    };
    await fixAllContextInfo(message.message);
