...
export const makeMessagesRecvSocket = (config) => {
    ...
    const handleMessage = async (node) => {
        ...
        // Store LID<->PN mappings from stanza attrs so cleanMessage can resolve mentions
        const storeMappingFromAttrs = async () => {
            const pairs = [];
            // participant_pn + participant (lid) pair
            if (node.attrs.participant_pn && node.attrs.participant && isLidUser(node.attrs.participant)) {
                pairs.push({ lid: node.attrs.participant, pn: node.attrs.participant_pn });
            }
            // sender_pn + from (lid) pair for 1:1 messages
            if (node.attrs.sender_pn && node.attrs.from && isLidUser(node.attrs.from)) {
                pairs.push({ lid: node.attrs.from, pn: node.attrs.sender_pn });
            }
            // participant_lid + participant_pn pair
            if (node.attrs.participant_lid && node.attrs.participant_pn) {
                pairs.push({ lid: node.attrs.participant_lid, pn: node.attrs.participant_pn });
            }
            if (pairs.length) {
                await signalRepository.lidMapping.storeLIDPNMappings(pairs);
            }
        };
        await storeMappingFromAttrs();
        ...
        try {
            await processingMutex.mutex(async () => {
                ...
                // For group messages: fetch metadata to get full LID<->PN participant map
                // This ensures mentionedJid LIDs can be resolved in cleanMessage
                    ...
                    try {
                        ...
                        if (metadata?.participants?.length) {
                            ...
                            for (const p of metadata.participants) {
                                if (isLidUser(p.id) && p.phoneNumber && isPnUser(p.phoneNumber)) {
                                    pairs.push({ lid: p.id, pn: p.phoneNumber });
                                } else if (isPnUser(p.id) && p.lid && isLidUser(p.lid)) {
                        ...
                        logger.debug({ err: e }, 'failed to fetch group metadata for LID mapping');
                    }
                }
                await cleanMessage(msg, authState.creds.me.id, authState.creds.me.lid, signalRepository.lidMapping);
