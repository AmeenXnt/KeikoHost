...
export const makeMessagesSocket = (config) => {
    ...
    const { ev, authState, processingMutex, messageMutex,signalRepository, upsertMessage, query, fetchPrivacySettings, sendNode, groupMetadata, groupToggleEphemeral } = sock;
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Support old-style Baileys buttons format (buttons array) and convert it into
            // interactiveButtons, so we can send native flow button messages.
            if (typeof content === 'object' && Array.isArray(content.buttons) && content.buttons.length > 0) {
                const interactiveButtons = content.buttons.map((b, i) => {
                    if (b?.nativeFlowInfo) {
                        return {
                    ...
                    const displayText = b?.buttonText?.displayText || b?.displayText || b?.buttonId || `Button ${i + 1}`;
                    const id = b?.buttonId || b?.id || displayText;
                ...
                // If the message contains media (image/video/document), use a hydrated template message
                // because some WhatsApp mobile clients can't render native-flow media button messages.
                const hasMedia = !!(content.image || content.video || content.document);
                if (hasMedia) {
                    const { text = '', caption = '', footer = '' } = content;
                    const hydratedButtons = interactiveButtons.map(b => ({ quickReplyButton: JSON.parse(b.buttonParamsJson) }));

                    // Prepare the media for upload
                    let pm;
                    if (content.image) {
                        const mi = Buffer.isBuffer(content.image) ? { image: content.image } : { image: { url: typeof content.image === 'object' ? content.image.url : content.image } };
                        pm = await prepareWAMessageMedia(mi, { upload: waUploadToServer });
                    }
                    else if (content.video) {
                        const mi = Buffer.isBuffer(content.video) ? { video: content.video } : { video: { url: typeof content.video === 'object' ? content.video.url : content.video } };
                        pm = await prepareWAMessageMedia(mi, { upload: waUploadToServer });
                    }
                    else if (content.document) {
                        const mi = Buffer.isBuffer(content.document) ? { document: content.document } : { document: { url: typeof content.document === 'object' ? content.document.url : content.document } };
                        pm = await prepareWAMessageMedia(mi, { upload: waUploadToServer });
                    }
                    ...
                    const hydratedTemplate = {
                        ...
                        hydratedButtons,
                        ...(pm?.imageMessage ? { hydratedImage: pm.imageMessage } : {}),
                        ...(pm?.videoMessage ? { hydratedVideo: pm.videoMessage } : {}),
                        ...(pm?.documentMessage ? { hydratedDocument: pm.documentMessage } : {})
                    };

                    const msg = generateWAMessageFromContent(jid, { templateMessage: { hydratedTemplate } }, { userJid, quoted: options?.quoted || null });
            ...
            if (typeof content === 'object' && Array.isArray(content.interactiveButtons) && content.interactiveButtons.length > 0) {
                ...
                const messageContentWrapper = viewOnce
                    ? { viewOnceMessage: { message: { interactiveMessage: payload } } }
                    : { interactiveMessage: payload };

                const msg = generateWAMessageFromContent(jid, messageContentWrapper, { userJid, quoted: options?.quoted || null });
            ...
            if (typeof content === 'object' &&
                ...
                if (config.emitOwnEvents) {
                    process.nextTick(async () => {
                        // messageMutex may be undefined in some versions/contexts (e.g. when using a patched socket).
                        // Fall back to calling upsertMessage directly when mutex is not available.
                        if (messageMutex?.mutex) {
